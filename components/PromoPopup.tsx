      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md bg-secondary-black border border-royal-gold/30 rounded-3xl p-8 pt-12 overflow-hidden shadow-2xl z-10 text-center"
          >
            {/* Close Button */}
            <button
              onClick={handleClose}
              className="absolute top-4 right-4 p-2 rounded-full text-zinc-300 hover:text-white hover:bg-white/10 transition-colors duration-200 focus:outline-none"
            ...
            <div className="flex flex-col gap-3">
              <Link
                href="/menu"
                onClick={handleClose}
                className="w-auto mx-auto inline-flex items-center justify-center gap-2 font-body text-xs font-bold uppercase tracking-wider py-3 px-6 rounded-full bg-royal-gold text-primary-black hover:bg-light-gold hover:scale-[1.02] transition-all duration-300 shadow-lg shadow-royal-gold/10 cursor-pointer"
              >
